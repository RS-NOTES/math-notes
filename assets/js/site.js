...
(function () {
  ...
  const I18N = {
    zh: {
      ...
      'home.geo': '几何速写', 'home.geo.sub': '微分几何与代数拓扑',
      ...
      'pdf.outline': '目录', 'pdf.thumbs': '缩略图', 'pdf.prev': '上一页', 'pdf.next': '下一页',
      'pdf.zoomout': '缩小', 'pdf.zoomin': '放大', 'pdf.fit': '适应宽度', 'pdf.double': '双页', 'pdf.full': '全屏',
      'pdf.loading': '正在加载 PDF…', 'pdf.err': 'PDF 加载失败，请刷新重试或直接下载。',
      'pdf.page': '第 {cur} / {tot} 页', 'pdf.nooutline': '此 PDF 没有书签目录。',
      'about.title': '关于',
    ...
    en: {
      ...
      'home.geo': 'Geometry Sketches', 'home.geo.sub': 'Differential geometry and algebraic topology',
      ...
      'pdf.outline': 'Outline', 'pdf.thumbs': 'Thumbnails', 'pdf.prev': 'Previous page', 'pdf.next': 'Next page',
      'pdf.zoomout': 'Zoom out', 'pdf.zoomin': 'Zoom in', 'pdf.fit': 'Fit width', 'pdf.double': 'Two pages', 'pdf.full': 'Fullscreen',
      'pdf.loading': 'Loading PDF…', 'pdf.err': 'Failed to load the PDF. Please refresh or download it.',
      'pdf.page': 'Page {cur} / {tot}', 'pdf.nooutline': 'This PDF has no bookmarks.',
      'about.title': 'About',
  ...
  const GEOFIGS = [
    ...
    { src: 'assets/figures/covering.svg', name: { zh: '复叠空间', en: 'Covering space' }, note: { zh: '复叠映射 p: ℝ→S¹ 与环路的提升', en: 'The covering map p: ℝ→S¹ and lifts of loops' } },
    ...
    { src: 'assets/figures/homotopy.svg', name: { zh: '基本群', en: 'Fundamental group' }, note: { zh: 'π₁(ℝ²∖{0})≅ℤ：α 绕洞一周，β 零伦', en: 'π₁(ℝ²∖{0})≅ℤ: α winds once, β is null-homotopic' } },
  ];
  ...
  function renderPdf() {
    ...
    el.innerHTML =
      ...
      manage +
      '</div>' +
      '<div class="pdf-app" id="pdfApp">' +
      '<div class="pdf-toolbar">' +
      '<div class="tb-group">' +
      '<button type="button" class="tb-btn" id="tbOutline" title="' + esc(t('pdf.outline')) + '">☰</button>' +
      '<button type="button" class="tb-btn" id="tbThumbs" title="' + esc(t('pdf.thumbs')) + '">▦</button>' +
      '</div>' +
      '<div class="tb-group">' +
      '<button type="button" class="tb-btn" id="tbPrev" title="' + esc(t('pdf.prev')) + '">‹</button>' +
      '<input id="tbPage" class="tb-page" inputmode="numeric" value="1" aria-label="page">' +
      '<span class="tb-total" id="tbTotal">/ 1</span>' +
      '<button type="button" class="tb-btn" id="tbNext" title="' + esc(t('pdf.next')) + '">›</button>' +
      '</div>' +
      '<div class="tb-group">' +
      '<button type="button" class="tb-btn" id="tbZoomOut" title="' + esc(t('pdf.zoomout')) + '">−</button>' +
      '<span class="tb-zoom" id="tbZoom">100%</span>' +
      '<button type="button" class="tb-btn" id="tbZoomIn" title="' + esc(t('pdf.zoomin')) + '">+</button>' +
      '<button type="button" class="tb-btn" id="tbFit">' + t('pdf.fit') + '</button>' +
      '<button type="button" class="tb-btn" id="tbDouble" title="' + esc(t('pdf.double')) + '">▥</button>' +
      '</div>' +
      '<div class="tb-group tb-right">' +
      '<button type="button" class="tb-btn" id="tbFull" title="' + esc(t('pdf.full')) + '">⛶</button>' +
      '</div>' +
      '</div>' +
      '<div class="pdf-body">' +
      '<aside class="pdf-side" id="pdfSide" hidden>' +
      '<div class="pdf-side-in" id="pdfSideIn"></div>' +
      '</aside>' +
      '<div class="pdf-scroll" id="pdfScroll">' +
      '<div class="pdf-loading" id="pdfLoading">' + esc(t('pdf.loading')) + '</div>' +
  ...
  /* 自托管 PDF.js 阅读器：目录、缩略图、缩放、双页、深色模式（无第三方资源） */
  async function initPdfViewer(p) {
    const app = $('#pdfApp'), scroll = $('#pdfScroll'), pagesEl = $('#pdfPages'),
      side = $('#pdfSide'), sideIn = $('#pdfSideIn'), loading = $('#pdfLoading');
    const dpr = Math.min(window.devicePixelRatio || 1, 2);
    const BASE = 1.4;
    let pdfjs = null, doc = null, pages = [], numPages = 0, cur = 1;
    ...
    $('#tbTotal').textContent = '/ ' + numPages;
    loading.hidden = true;
    pagesEl.hidden = false;

    const effScale = () => {
      if (fitMode && pages.length) {
        const w = Math.max(pagesEl.clientWidth - 32, 200);
        return Math.max(0.4, w / pages[0].vp1.width);
      }
      return BASE * zoom;
    };
    const zoomText = () => (fitMode ? t('pdf.fit') : Math.round(effScale() * 100) + '%');

    async function makePage(i) {
      ...
      return { i, pg, vp1, div, canvas, ctx: canvas.getContext('2d'), scale: 0, rendered: false };
    }

    async function renderAt(P, s) {
      if (!P || P.scale === s) return;
      const vp = P.pg.getViewport({ scale: s * dpr });
      P.canvas.width = vp.width;
      P.canvas.height = vp.height;
      P.canvas.style.width = Math.round(vp.width / dpr) + 'px';
      P.canvas.style.height = Math.round(vp.height / dpr) + 'px';
      await P.pg.render({ canvasContext: P.ctx, viewport: vp, transform: dpr !== 1 ? [dpr, 0, 0, dpr, 0, 0] : null }).promise;
      P.scale = s;
    }

    async function renderAll() {
      const s = effScale();
      $('#tbZoom').textContent = zoomText();
      for (const P of pages) if (P.rendered) await renderAt(P, s);
    }

    const obs = new IntersectionObserver((entries) => {
      entries.forEach((en) => {
        if (!en.isIntersecting) return;
        const P = pages[Number(en.target.dataset.idx)];
        P.rendered = true;
        renderAt(P, effScale());
      });
    ...
    for (let i = 0; i < numPages; i++) {
      const P = await makePage(i);
      P.rendered = i < 2;
      pages.push(P);
      if (i < 2) renderAt(P, effScale());
      obs.observe(P.div);
    }
    applyTwoUp();

    function applyTwoUp() {
      pagesEl.classList.toggle('two-up', twoUp);
      pages.forEach((P) => P.div.classList.toggle('spread-full', twoUp && P.i === 0));
    }

    function goto(i, instant) {
      const n = Math.min(Math.max(i, 1), numPages);
      const div = pages[n - 1].div;
      scroll.scrollTo({ top: div.offsetTop - 6, behavior: instant ? 'auto' : 'smooth' });
      cur = n;
      $('#tbPage').value = String(n);
    }

    scroll.addEventListener('scroll', () => {
      let best = 1, bestTop = Infinity;
      for (const P of pages) {
        const top = Math.abs(P.div.offsetTop - scroll.scrollTop - 30);
        if (top < bestTop) { bestTop = top; best = P.i + 1; }
      }
      if (best !== cur) { cur = best; $('#tbPage').value = String(best); }
    }, { passive: true });

    $('#tbPage').addEventListener('change', () => {
      const v = parseInt($('#tbPage').value, 10);
      if (v >= 1 && v <= numPages) goto(v);
      else $('#tbPage').value = String(cur);
    });
    $('#tbPrev').addEventListener('click', () => goto(cur - 1));
    $('#tbNext').addEventListener('click', () => goto(cur + 1));
    $('#tbZoomIn').addEventListener('click', () => { fitMode = false; zoom = Math.min(zoom * 1.25, 3); renderAll(); });
    $('#tbZoomOut').addEventListener('click', () => { fitMode = false; zoom = Math.max(zoom * 0.8, 0.5); renderAll(); });
    $('#tbFit').addEventListener('click', () => { fitMode = !fitMode; renderAll(); });
    $('#tbDouble').addEventListener('click', () => { twoUp = !twoUp; applyTwoUp(); if (fitMode) renderAll(); });
    ...
    /* 侧栏：目录 / 缩略图 */
    function sideHTML(html) {
      sideIn.innerHTML = '<div class="pdf-side-head">' + (sideMode === 'outline' ? t('pdf.outline') : t('pdf.thumbs')) + '</div>' + html;
      side.hidden = false;
    ...
    $('#tbOutline').addEventListener('click', async () => {
      if (sideMode === 'outline') { closeSide(); sideMode = null; return; }
      sideMode = 'outline';
      let items = null;
      try { items = await doc.getOutline(); } catch (e) { items = null; }
      if (!items || !items.length) { sideHTML('<p class="empty">' + esc(t('pdf.nooutline')) + '</p>'); return; }
      const mk = (arr) =>
        '<ol class="pdf-ol">' + arr.map((it) =>
          '<li><button type="button" data-dest="' + esc(JSON.stringify(it.dest)) + '">' + esc(it.title) + '</button>' +
          (it.items && it.items.length ? mk(it.items) : '') + '</li>').join('') + '</ol>';
      sideHTML(mk(items));
      $('.pdf-ol button', sideIn).forEach((b) => {
        b.addEventListener('click', async () => {
          let dest = null;
          try { dest = JSON.parse(b.dataset.dest); } catch (e) { return; }
          try {
    ...
    $('#tbThumbs').addEventListener('click', () => {
      if (sideMode === 'thumbs') { closeSide(); sideMode = null; return; }
      sideMode = 'thumbs';
      sideHTML('<div id="thumbsBox"></div>');
      const box = $('#thumbsBox');
      const tob = new IntersectionObserver((es) => {
        es.forEach((en) => {
          if (!en.isIntersecting) return;
          const P = pages[Number(en.target.dataset.idx)];
          if (P.thumbDone) return;
          P.thumbDone = true;
          const vp = P.pg.getViewport({ scale: 110 / P.vp1.width });
          const c2 = document.createElement('canvas');
          c2.width = vp.width; c2.height = vp.height;
          P.pg.render({ canvasContext: c2.getContext('2d'), viewport: vp });
          en.target.insertBefore(c2, en.target.firstChild);
        });
      }, { root: side, rootMargin: '400px 0px' });
      pages.forEach((P) => {
        const d = document.createElement('div');
        d.className = 'pdf-thumb';
        d.dataset.idx = String(P.i);
        const s = document.createElement('span');
        s.textContent = String(P.i + 1);
        d.appendChild(s);
        d.addEventListener('click', () => goto(P.i + 1));
        box.appendChild(d);
        tob.observe(d);
      });
    });

    window.addEventListener('resize', () => { if (fitMode) renderAll(); });
    document.addEventListener('keydown', (e) => {
      if (page !== 'pdf' || !doc) return;
